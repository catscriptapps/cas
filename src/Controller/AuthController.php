...
class AuthController
{
    ...
    public static function forgotPassword(array $input): array
    {
        ...
        // 1. Check if an account exists (any account type)
        $account = self::findAccountByEmail($email);
        ...
        try {
            ...
            // Local dev: skip sending the email entirely and send the caller
            // straight to the reset page with a valid token already issued --
            // same token generation/hashing/storage as production above, just
            // no MailService call, so resetPassword() validates it through the
            // exact same path either way. Production is untouched.
            if (($_ENV['APP_ENV'] ?? '') === 'local') {
                static::logActivity("Password reset requested (local bypass, no email sent)", 'Auth', $account->id);

                return [
                    'success' => true,
                    'message' => 'Local environment detected — redirecting you straight to the reset page.',
            ...
            static::logActivity("Password reset email sent", 'Auth', $account->id);
    ...
    public static function login(array $input): array
    {
        ...
        $returnTo = trim($input['return_to'] ?? '') ?: null;
        ...
        try {
            ...
            $result = AuthService::login($email, $password, $returnTo);
        } catch (\Throwable $e) {
    ...
    public static function resetPassword(array $input): array
    {
        ...
        try {
            ...
            // 4. Update whichever account owns this email
            $account = self::findAccountByEmail($email);
            if (!$account) {
                return [
                    'success' => false,
            ...
            static::logActivity("Password updated via reset link", 'Auth', $account->id);
