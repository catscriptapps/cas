...
<tr id="league-row-<?= $rowItem['league_id'] ?? '0' ?>" data-encoded-id="<?= $rowItem['encoded_id'] ?? '' ?>" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group font-sans">
    <td class="px-6 py-4">
        <div class="flex items-center justify-between gap-3">
            <div class="min-w-0">
                <div class="text-sm font-bold text-gray-900 dark:text-white truncate"><?= htmlspecialchars($rowItem['league'] ?? '') ?></div>
                <div class="text-xs text-gray-400 sm:hidden"><?= htmlspecialchars($rowItem['sport_name'] ?? 'N/A') ?> &middot; <?= (int)($rowItem['division_count'] ?? 0) ?> divisions &middot; <?= $statusBadge ?></div>
            </div>
            <?php if ($canManage): ?>
