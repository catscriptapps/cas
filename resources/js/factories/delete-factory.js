...
export function createDeleteHandler(endpoint, resourceName) {
    ...
    if (!modal) {
        ...
        modal.id = modalId;
        // z-[10020]: must outrank the fixed topbar (z-[9999]), the detail
        // view modals it's commonly opened from within (z-[10000]), and the
        // upload/preview overlays (z-[10010]) -- a delete confirmation should
        // always be the topmost thing on screen when it's up.
        modal.className = 'fixed inset-0 z-[10020] flex items-center justify-center p-4 bg-gray-900 bg-opacity-70 dark:bg-opacity-80 transition-opacity duration-300 opacity-0 pointer-events-none';
        modal.innerHTML = `
