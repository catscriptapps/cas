 ...
 * @param {(files: Array) => void} onComplete
 * @param {Object} [extraFields] Extra form fields sent with every chunk (e.g. a parent record's id).
 */
export function createVideoUploadHandler(endpointUrl, onComplete, extraFields = {}) {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (finalResponse && finalResponse.success) {
                ...
                setTimeout(() => {
                    // Without this, selecting the same file again on a later
                    // upload doesn't fire a fresh 'change' event (the input's
                    // value never changed), silently no-oping the next upload.
                    input.value = '';
