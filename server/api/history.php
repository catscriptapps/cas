...
try {
    // --------------------------------------------------
    // GET: Filter / Sort / Pagination (see index()'s docblock for the
    // full query-string contract shared with resources/js/components/data-table.js)
    // --------------------------------------------------
    if ($method === 'GET') {
        (new RecentActivitiesController())->index();
        exit;
    }
